...
public class PathController {
	...
	@PutMapping("active")
	public void setActivePath(@AuthenticatedSCPerson SCPerson person, @ParamEntity SCEdition edition,
			@ParamEntity Path path) {
		pathService.setActivePath(person, edition, path);
	}

	@GetMapping("customisation")
	public PathCustomisationView getPathCustomisation(@AuthenticatedSCPerson SCPerson person,
			@ParamEntity SCEdition edition) {
		return pathService.getPathCustomisation(person, edition);
	}

	@PostMapping("tasks/{task}")
	public void addTaskToPath(@AuthenticatedSCPerson SCPerson person, @PathEntity Task task) {
		pathService.addTaskToPath(person, task);
	}

	@DeleteMapping("tasks/{task}")
	public void removeTaskFromPath(@AuthenticatedSCPerson SCPerson person, @PathEntity Task task) {
		pathService.removeTaskFromPath(person, task);
	}
	...
	@PatchMapping("{path}")
	@PreAuthorize("@authorisationService.canEditEditionCircuit(#path.edition.id)")
