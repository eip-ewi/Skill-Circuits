...
public class TaskService {
    ...
    public void moveTask(Task task, TaskMove move) {
        task.getSkill().getTasks().stream().filter(t -> t.getIdx() > task.getIdx()).forEach(t -> t.setIdx(t.getIdx() - 1));
        taskRepository.saveAll(task.getSkill().getTasks());

        Skill newSkill = dtoConverter.apply(move.getSkill());
        newSkill.getTasks().stream().filter(t -> t.getIdx() >= move.getIndex()).forEach(t -> t.setIdx(t.getIdx() + 1));
    ...
    public TaskInfo moveTaskInsideChoiceTask(ChoiceTask choiceTask, RegularTask subtask) {
        subtask.getSkill().getTasks().stream().filter(t -> t.getIdx() > subtask.getIdx()).forEach(t -> t.setIdx(t.getIdx() - 1));
