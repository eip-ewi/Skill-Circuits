...
public class EditionCircuitService {
    ...
    public EditionLevelEditionView getEditionCircuit(Long editionId, SCPerson person) {
        ...
        return new EditionLevelEditionView(
                ...
                edition.getModules().stream().map(module -> convertToModuleView(module, completedTaskIds)).toList(),
                edition.getCheckpoints().stream().map(checkpoint -> new CheckpointView(checkpoint.getId(), checkpoint.getName(), checkpoint.getDeadline())).toList(),
                edition.getPaths().stream().map(path -> new PathView(path.getId(), path.getName(), path.getDescription())).toList()
        );
    }
    ...
    private EditionLevelModuleView convertToModuleView(SCModule module, Set<Long> completedTaskIds) {
        return new EditionLevelModuleView(
                module.getId(),
                module.getName(),
                module.getSubmodules().stream().map(submodule -> convertToSubmoduleView(submodule, completedTaskIds)).toList()
        );
    }
    ...
    private EditionLevelSubmoduleView convertToSubmoduleView(Submodule submodule, Set<Long> completedTaskIds) {
        return new EditionLevelSubmoduleView(
                submodule.getId(),
                submodule.getName(),
                submodule.getColumn(),
                submoduleDependencyService.getSubmoduleParents(submodule).stream().map(Submodule::getId).toList(),
                submoduleDependencyService.getSubmoduleChildren(submodule).stream().map(Submodule::getId).toList(),
                submodule.getSkills().stream().map(skill -> convertToSkillView(skill, completedTaskIds)).toList()
        );
    }
    ...
    private EditionLevelSkillView convertToSkillView(Skill skill, Set<Long> completedTaskIds) {
        return new EditionLevelSkillView(
                ...
                skill.getName(),
                skill.isEssential(),
                skillStateService.isSkillCompletedBy(skill, completedTaskIds),
                !skillStateService.isSkillUnlockedFor(skill, completedTaskIds)
        );
    }
