...
public class PersonService {
	...
	public Optional<PathPreference> getPathForEdition(Long personId, Long editionId) {
		...
		Optional<PathPreference> p = scPerson.getPathPreferences().stream()
				.filter(x -> x.getEditionId().equals(editionId)).findFirst();
		return p;
	}
	 ...
	 */
	@Transactional
	public SCPerson getOrCreateSCPerson(Long id) {
		return personRepository.findById(id)
				.orElseGet(() -> personRepository.save(SCPerson.builder().id(id).build()));
	}
}
