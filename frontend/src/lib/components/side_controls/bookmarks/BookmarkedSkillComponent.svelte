...
<div
    ...
    <div
        bind:this={element}
        class="block"
        draggable={hasEditorRights() &&
            isLevel(ModuleLevel) &&
            !getBlocks().some(b => b.id === skill.id)}
        ondragstart={dragStart}
        ondragend={dragEnd}
        data-clickable={!hasEditorRights()}
        onclick={click}
        data-completed={!hasEditorRights() && isCompleted(skill)}>
        <BlockContentComponent block={skill} completed={!hasEditorRights() && isCompleted(skill)}></BlockContentComponent>
    </div>
