...
<div class="item" {draggable} ondragstart={dragStart} ondragend={dragEnd}>
    ...
    <BookmarkMenuComponent
        ...
        onLists={getBookmarks().filter(list =>
            list.tasks.some(t => t.taskType === "regular" && t.infoId === taskInfo.infoId),
        )}
        addToList={list => addTaskInfoToBookmarkList(taskInfo, list)}
        removeFromList={list => removeTaskInfoFromBookmarkList(taskInfo, list)}>
        <Button primary square aria-label="Bookmark" onclick={() => (bookmarksOpen = true)}>
            <span
                class="fa-bookmark"
                class:fa-regular={!isTaskInfoBookmarked(taskInfo)}
                class:fa-solid={isTaskInfoBookmarked(taskInfo)}>
            </span>
        </Button>
    </BookmarkMenuComponent>
    ...
    {#if taskInfo.taskType === "choice"}
        <WithConfirmationDialog
            onconfirm={() => deleteSubtask(taskInfo)}
