<script lang="ts">
    ...
    import {
        connectSkills,
        disconnectSkills,
    } from "../../../logic/circuit/updates/connection_updates.svelte";
    ...
    let {
        skill,
        action = $bindable(),
        connectable = $bindable(),
    }: { skill: SkillBlock; action: BlockAction | undefined; connectable: boolean } = $props();
    ...
    $effect(() => {
        ...
        if (skill.state === BlockStates.WaitingForConnection) {
            let alreadyConnected =
                getGraph().isAncestor(connectingTo, skill) ||
                getGraph().isDescendant(connectingTo, skill);
            ...
            if (
                ancestors.some(
                    ancestor => getGraph().getShortestPathToAncestor(skill, ancestor)!.length == 2,
                )
            ) {
                connectUp = false;
            }
            if (
                ancestors.some(
                    ancestor =>
                        getGraph().getShortestPathToAncestor(connectingTo, ancestor)!.length == 2,
                )
            ) {
                connectDown = false;
            }
    ...
    function addParent() {
        let connectingTo = getBlocks().find(
            other => other.state === BlockStates.Connecting,
        ) as SkillBlock;
    ...
    function removeParent() {
        let disconnectingFrom = getBlocks().find(
            other => other.state === BlockStates.Connecting,
        ) as SkillBlock;
...
<div class="controls" transition:transition>
    {#if connectUp}
        <Button
            square
            aria-label="Add as dependency"
            onclick={addParent}
            onmouseenter={() => (action = BlockActions.AddParent)}
    ...
    {:else if disconnectUp}
        <Button
            ...
            onmouseenter={() => (action = BlockActions.RemoveParent)}
            onmouseleave={() => (action = undefined)}>
    ...
    {#if connectDown}
        <Button
            square
            aria-label="Add as dependant"
            onclick={addChild}
            onmouseenter={() => (action = BlockActions.AddChild)}
            onmouseleave={() => (action = undefined)}>
    ...
    {:else if disconnectDown}
        <Button
            ...
            onclick={removeChild}
            onmouseenter={() => (action = BlockActions.RemoveChild)}
            onmouseleave={() => (action = undefined)}>
            <span class="fa-solid fa-link-slash"></span>
        </Button>
    {:else}
