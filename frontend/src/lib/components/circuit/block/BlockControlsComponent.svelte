...
<div
    ...
    {#if block.state === BlockStates.Connecting}
        <Button
            square={true}
            aria-label="Stop editing connections"
            onclick={stopConnecting}
            onmouseenter={() => (action = BlockActions.CancelLink)}
    ...
    {:else if block.state === BlockStates.Editing}
        <Button
            ...
            onclick={stopEditing}
            onmouseenter={() => (action = BlockActions.StopEdit)}
            onmouseleave={() => (action = undefined)}>
    ...
    {:else}
        ...
        {#if isLevel(ModuleLevel)}
            <Button
                ...
                onmouseenter={() => (action = BlockActions.Link)}
                onmouseleave={() => (action = undefined)}>
        ...
        <Button
            ...
            aria-label="Edit"
            onclick={edit}
            onmouseenter={() => (action = BlockActions.Edit)}
            onmouseleave={() => (action = undefined)}>
        ...
        <WithConfirmationDialog
            ...
            {#snippet button(showDialog: () => void)}
                <Button
                    ...
                    aria-label="Delete"
                    onclick={showDialog}
                    onmouseenter={() => (action = BlockActions.Delete)}
                    onmouseleave={() => (action = undefined)}>
